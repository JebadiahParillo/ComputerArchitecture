...
public class Assembler {
    public static String[] assemble(String[] instruction) throws Exception {
        ...
        for(int i = 0; i<instruction.length; i++) {
            ...
            else if (splitArray[0].equals("jump") || splitArray[0].equals("call")) {
                int address = Integer.parseInt(splitArray[1]);
                if(splitArray[0].equals("jump")) {
                    longword jump_longword = new longword(address).leftShift(20);
                    preloadString[i] = "0011" + stringBuilder(jump_longword, 12);
                } else if (splitArray[0].equals("call")) {
                    longword callAddress = new longword(address).leftShift(22);
                    preloadString[i] = "011010" + stringBuilder(callAddress, 10);
                }
            }
            ...
            else if(splitArray[0].equals("push") || splitArray[0].equals("pop")){
                 if(splitArray[1].length() > 2) {
                    //this is getting the storage register
                     String registerBit = Character.toString(splitArray[1].charAt(1))+Character.toString(splitArray[1].charAt(2));
                     longword registerLongword = new longword(Integer.parseInt(registerBit)).leftShift(28);
                     if(splitArray[0].equals("push")){
                         preloadString[i] = "011000000000" + stringBuilder(registerLongword,4);
                     }
                     else if (splitArray[0].equals("pop")) {
                         preloadString[i] = "011001000000" + stringBuilder(registerLongword,4);
                     }
                 }
                else {
                     String registerBit = Character.toString(splitArray[1].charAt(1));
                     longword registerLongword = new longword(Integer.parseInt(registerBit)).leftShift(28);
                     if(splitArray[0].equals("push")){
                         preloadString[i] = "011000000000" + stringBuilder(registerLongword,4);
                     }
            ...
            else if(splitArray[0].equals("return")){
                preloadString[i] = "0110110000000000";
            }
