public class computer {
    ...
    public void decode() throws Exception {
        ...
        //when the instruction is a comparison we need op3 or uses Stack
    ...
    public void execute() throws Exception {
        ...
        //The instruction is an interrupt
            ...
            longword op3_shifted = op3_masked.rightShift(16);
            op3 = registers[op3_shifted.getSigned()];
            bit b1 = currentInstruction.getBit(4);
            bit b2 = currentInstruction.getBit(5);
            if(!b1.getValue() && !b2.getValue()){
                computer_mem.write(SP,op3);
                ...
                setRegisters(op3.getSigned(),computer_mem.read(SP));
                //pop
            }
            else if (b1.getValue() && !b2.getValue()) {
                longword temp;
                computer_mem.write(SP,PC);
                if(currentInstruction.getBit(25).getValue()){
                    temp = currentInstruction.and(new longword(67043328)).rightShift(16);
                    for(int i = 0; i<24; i++){
                        temp.setBit(i,new bit(true));
                    }
                }
